...
final class PromptBuilder {
    ...
    static String build(Game game) {
        ...
        return """
                ...
                스타 선수 출전 여부, 한국어 중계처 등)를 수집한 뒤 분석하라. 내부 지식만으로
                추측하지 마라. 위 경기가 분석 대상이다 — 다른 경기를 찾지 말 것.

                ## 한국어 중계처 판단
                한국 스포츠 중계는 보통 다음 플랫폼에서 한다:
                KBS, SBS, MBC 및 스포츠 채널(KBS N SPORTS, SBS Sports, MBC SPORTS+), SPOTV,
                쿠팡플레이, 티빙(TVING), 아마존 프라임 비디오, 유튜브, 치지직.
                웹검색으로 이 경기를 위 플랫폼(또는 다른 공인 한국 중계처)에서 한국어로 중계하는지
                확인하라. 어디서도 한국어 중계를 찾을 수 없으면 보통 한국어 중계가 없는 것이다.
                ...
                ## 출력 형식 (반드시 이 JSON만 출력, 마크다운 코드펜스/설명 금지)
                {
                  ...
                  "koreanBroadcast": "한국어 중계처 (예: 쿠팡플레이, SBS Sports, 치지직). 여러 곳이면 쉼표로. 못 찾으면 '한국어 중계 없음'",
